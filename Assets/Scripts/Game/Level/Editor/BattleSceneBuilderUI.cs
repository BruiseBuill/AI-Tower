...
{
    /// <summary>
    /// BattleSceneBuilder 的 UI 部分：战斗画布、三档能量条、倒计时、部署状态、暂停与结算面板。
    ...
    public static partial class BattleSceneBuilder
    {
        ...
        static BattleHUD CreateHud()
        {
            ...
            // ---- 底部当前部署档位 ----
            GameObject deployStatusObject = CreateUiObject("DeployStatus", canvasRect);
            Text deployStatusLabel = AddText(deployStatusObject, "突击兵  消耗 50", 30,
                TextAnchor.MiddleCenter, Color.white);
            Anchor(deployStatusObject.GetComponent<RectTransform>(), new Vector2(0.5f, 0f),
            ...
            SetSerialized(hud, so =>
            {
                ...
                so.FindProperty("deployStatusLabel").objectReferenceValue = deployStatusLabel;
